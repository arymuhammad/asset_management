...
import 'widget/add_edit_adj_in.dart';

class AdjusmentIn extends GetView<AdjusmentController> {
  AdjusmentIn({super.key, this.userData});
  final Data? userData;
      ...
      builder: (context, constraints) {
        ...
        return Scaffold(
          body: Padding(
            ...
            child: FutureBuilder(
              future: adjC.getAdjInData(userData!.kodeCabang!),
              builder: (context, snapshot) {
                if (snapshot.hasData) {
                  adjC.dtAdjSource = AdjInDataSource(
                    dataAdjIn: adjC.dataAdjInFiltered,
                  ...
                  return PaginatedDataTable2(
                    ...
                    actions: [
                      Visibility(
                        ...
                        child: SizedBox(
                          ...
                          child: CsTextField(
                            ...
                            label: 'Search Data',
                            onChanged: (val) {
                              controller.filterAdjIn(val);
                              // ignore: invalid_use_of_protected_member, invalid_use_of_visible_for_testing_member
                              controller.dtAdjSource.notifyListeners();
                            },
                          ),
                        ),
                      ),
                      IconButton(
                        onPressed: () async {
                          await adjC.generateNumbId(adjIn: true);
                          addEditAdjIn(
                            context,
                            '',
                            '',
...
class AdjInDataSource extends DataTableSource {
  AdjInDataSource({required this.dataAdjIn, required bool screenWidth});
  final RxList<AdjModel> dataAdjIn;
  ...
  DataRow? getRow(int index) {
    ...
    if (index >= dataAdjIn.length) {
      return null;
    }
    final item = dataAdjIn[index];
                ...
                child: IconButton(
                  ...
                  onPressed: () async {
                    promptDialog(
                      ...
                      () async {
                        await adjC.deleteAdjIn(item.id!);
                      },
                      isWideScreen,
                    );
                  },
  ...
  @override
  int get rowCount => dataAdjIn.length;
